...
books.forEach((book) => {
  booksHTML += `
      ...
      <div class="grid-item">
        ...
        <div class="Btn-div">
          <button class="Btn read-online-btn" onclick="location.href='${book.pdf}'">
          ...
          <form method="post" id="libraryform" action="myLibrary.php" >
            ...
            <input type="hidden" name="book_pdf" value="${book.pdf}">
            <input type="hidden" name="book_title" value="${book.title}">
            <input type="hidden" name="book_author" value="${book.author}">
            <input type="hidden" name="book_desc" value="${book.desc}">
          </form>
        </div>
      </div>
  `;
...
function catHTML(cat) {
  books.forEach((book) => {
    if(book.cat == cat) {
      ...
      document.querySelector('.js-book-cat-grid').innerHTML = catBooksHTML;
    }
  });
}

let self_help = document.getElementsByClassName('self-help'); 
self_help[0].addEventListener('click', catHTML(`self-help`));

let rom = document.getElementsByClassName('rom'); 
rom[0].addEventListener('click', catHTML(`romance`));

// let self_help = document.querySelector('.self-help'); 
// self_help.addEventListener('click', catHTML(`self-help`));
